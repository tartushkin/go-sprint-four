...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	СaloriesSpentRunning := ((runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return СaloriesSpentRunning
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	speedInMetersSecond := meanSpeed(action, duration) * kmhInMsec
	CaloriesSpentWalking := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speedInMetersSecond, 2)/heightInMeters)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return CaloriesSpentWalking
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	СaloriesSpentSwimming := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return СaloriesSpentSwimming
}
